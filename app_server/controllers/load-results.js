...
module.exports.loadResults = function (req, res) {
  ...
  query[0] = "SELECT COUNT(*) as totalResponse FROM `learning-for-purpose.analytics.responses` WHERE employment_status = '" + employStatus + "' AND abn_hash = '"+ orgABNhash + "' AND survey_id = '" + surveyID + "' AND question_id = '" + questionID +"' AND gender = '" + gender + "' AND timestamp BETWEEN '" + startDate + "' AND '" + endDate + "' AND year_of_birth BETWEEN "+ birthStart + " AND " + birthEnd + "; ";

  // Query 1: Organisation's average overall sentiment score for the question
  query[1] = "SELECT AVG(overall_sentiment) AS organizationAverage FROM `learning-for-purpose.analytics.responses` WHERE employment_status = '" + employStatus + "' AND abn_hash = '" + orgABNhash + "' AND survey_id = '" + surveyID + "' AND question_id = '" + questionID + "' AND gender = '" + gender + "' AND timestamp BETWEEN '" + startDate + "' AND '" + endDate + "' AND year_of_birth BETWEEN " + birthStart + " AND " + birthEnd + " ;";
  
  // Query 2: Get overall sentiment time-series data:
  query[2] = "SELECT timestamp as ds, AVG(overall_sentiment) as avgOs FROM `learning-for-purpose.analytics.responses` WHERE employment_status = '" + employStatus + "' AND abn_hash = '" + orgABNhash + "' AND survey_id = '" + surveyID + "' AND question_id = '" + questionID + "'  AND gender = '" + gender + "' AND year_of_birth BETWEEN " + birthStart + " AND " + birthEnd + "  GROUP BY ds ORDER BY ds;";

  // Query 3: Frequency count array of sentiment scores -10 to 10 (for Histogram by Score)
  query[3] = "SELECT overall_sentiment, COUNT(*) as frequency FROM `learning-for-purpose.analytics.responses` WHERE employment_status = '" + employStatus + "' AND abn_hash = '" + orgABNhash + "' AND survey_id = '" + surveyID + "' AND question_id = '" + questionID + "' AND gender = '" + gender + "' AND timestamp BETWEEN '" + startDate + "' AND '" + endDate + "' AND year_of_birth BETWEEN " + birthStart + " AND " + birthEnd + " GROUP BY overall_sentiment ORDER BY overall_sentiment ASC;";

  // Query 4: Get list of entities for entity search function
  query[4] = "SELECT DISTINCT LOWER(e.name) as ent FROM `learning-for-purpose.analytics.responses` R,UNNEST(entity) AS e WHERE abn_hash = '" + orgABNhash + "' AND survey_id = '" + surveyID + "' AND question_id = '" + questionID + "' AND gender = '" + gender + "' AND employment_status = '" + employStatus + "' AND timestamp BETWEEN '" + startDate + "' AND '" + endDate + "' AND year_of_birth BETWEEN " + birthStart + " AND " + birthEnd + ";";

  // Query 6: National average overall sentiment score for the question
  // query[1] = "SELECT AVG(overall_sentiment) AS nationalAverage FROM `cits-3200.analytics.responses` WHERE employment_status = '" + employStatus + "' AND timestamp BETWEEN '" + startDate + "' AND '" + endDate + "' AND question_id = '" + questionID + "' AND gender = '" + gender + "' AND year_of_birth BETWEEN " + birthStart + " AND " + birthEnd + " ;";
